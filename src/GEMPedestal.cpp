...
#elif defined(USE_SSP)
    MPDSSPRawEventDecoder *mpd_ssp_decoder[NTHREAD];
#elif defined(USE_SRS)
    SRSRawEventDecoder *srs_decoder[NTHREAD];
#else
    std::cout<<"Error: decoder type not defined..."<<std::endl;
    exit(0);
#endif
...
#elif defined(USE_SSP)
        mpd_ssp_decoder[i] = new MPDSSPRawEventDecoder();
        event_parser[i]->RegisterRawDecoder(static_cast<int>(Bank_TagID::MPD_SSP), mpd_ssp_decoder[i]);
#elif defined(USE_SRS)
        srs_decoder[i] = new SRSRawEventDecoder();
        event_parser[i]->RegisterRawDecoder(static_cast<int>(Bank_TagID::SRS), srs_decoder[i]);
#endif
    }
        ...
        uint32_t &nEvents)
{
    ...
    while(run)
    {
        ...
        [[maybe_unused]] auto & decoded_data = dynamic_cast<MPDSSPRawEventDecoder*>(
            ...
            ->GetAPV();
#elif defined(USE_SRS)
        [[maybe_unused]] auto & decoded_data = dynamic_cast<SRSRawEventDecoder*>(
                event_parser->GetRawDecoder(static_cast<int>(Bank_TagID::SRS)))
            ->GetAPV();
#endif
        
        // cleanup srs apv data headers
        std::unordered_map<APVAddress, std::vector<int>> _decoded_data;
        for(auto &i: decoded_data) {
            _decoded_data[i.first] = cleanup_srs_apv_header_words(i.second);
        }

        CalculateEventRawPedestal(_decoded_data);
...
std::vector<int> GEMPedestal::cleanup_srs_apv_header_words(const std::vector<int> &apv)
{
    std::vector<int> res;
